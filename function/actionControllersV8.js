...
// admin server url https://script.google.com/macros/s/AKfycbzRN1HCKWKpOyQsLXNzzopl0JZ8v2HD8VF2PBEEHjVq1_Wi9l0D71m-IMHA7DT1cmcsfg/exec

const scriptURL = `https://script.google.com/macros/s/AKfycbzRN1HCKWKpOyQsLXNzzopl0JZ8v2HD8VF2PBEEHjVq1_Wi9l0D71m-IMHA7DT1cmcsfg/exec`;
...
class ActionController extends ActionEvent {
  ...
  onRouteChange(e) {
    ...
    if (routeKeyword) {
      ...
      var routeModel = operate.findMatchingInArrayOfObject(
      ...
      if (routeModel.length != 0) {
        this.view.replaceChild([
          routeModel[0].model,
          this.view._actionView.entity,
        ]);
      } else {
  ...
  async onClick(event) {
    ...
    if (event.target.hasAttribute("data-command")) {
      ...
      switch (commandJson[0].command) {
        ...
        case "SubmitInvoice":
          event.preventDefault();
          await this.SubmitInvoice(event);
          break;
        case "NewItem":
          event.preventDefault();
        ...
        case "form":
          ...
          event.preventDefault();
          await actionengine.processRequest("viewFormRequest", {
            event: event,
            entity: commandJson[0].entity,
          });
          break; // await engine.processReq(viewFormRequest,{'event':event,'entity':commandJson[0].entity});
        case "modal":
          await ActionView.viewModal(event);
          break;
        case "closeModal":
          await ActionView.closeModal(event);
          break;
        //sheet
        case "ActionStories":
          alert("action stories case");
          event.preventDefault();
          actionengine.processRequest("getActionStoriesRequest");
          break; //await engine.processReq(GetActionStoriesFlowRequest);
        case "importFromSheet":
          event.preventDefault();
          await actionengine.processRequest("importFromSheetRequest");
          break; //await engine.processReq(importFromSheetFlowRequest);
        case "exportToSheet":
          alert("export to sheet case");
          event.preventDefault();
          await actionengine.processRequest("exportToSheetRequest");
          break; //awaitengine.processReq(exportToSheetFlowRequest)
        case "SearchFolder_Google":
          alert("get google drive folder case");
          event.preventDefault();
          await actionengine.processRequest("getGoogleDriveFolderRequest");
        ...
        case "new":
          event.preventDefault();
          await actionengine.processRequest("newFileRequest");
          break; //await engine.processReq(newActionStoryRequest);
        case "OpenFile":
          event.preventDefault();
          await actionengine.processRequest("OpenAFileRequest");
          break;
        case "OpenDirectory":
          event.preventDefault();
          await actionengine.processRequest("AddACollectionRequest");
          break;
        case "file":
          event.preventDefault();
          await actionengine.processRequest("EachFileRequest", {
            event: event,
            from: event.target.getAttribute("from"),
          });
          break; //await engine.processReq(everyFileRequest,{"event":event,'from':event.target.getAttribute('from')});
        case "FS_Save":
          event.preventDefault();
          await actionengine.processRequest("saveFileRequest");
          break; // await engine.processReq(saveFileFlowRequest);
        case "Redirect":
          event.preventDefault();
          await ActionController.onChangeRoute(commandJson[0].entity);
          break;
        default:
        // console.log("I don't know such values",event.type);
      }
    ...
    try {
      ...
      for (var i = 0; i < children.length; i++) {
        var item = [
          ...
          document.getElementsByClassName("DetailType")[i].textContent,
          document.getElementsByClassName("Ref")[i].textContent,
          document.getElementsByClassName("Account")[i].textContent,
          document.getElementsByClassName("LineStatus")[i].textContent,
        ];
        InvoiceItems.push(item);
      }
      var json = { array: InvoiceItems };
      ActionView.addInnerHTML("", document.getElementById("viewForm"));
      var response = await HttpService.fetchRequest(
        scriptURL,
        HttpService.requestBuilder("POST", undefined, json)
      );
      alert(response.output);
      ActionView.addInnerHTML(
        sampleIntroStory,
        document.getElementById("inlineContent")
      );
    } catch (err) {
      console.log(err);
    }
  ...
  async SendData(event) {
    try {
      ...
      var response = await HttpService.fetchRequest(
        scriptURL,
        HttpService.requestBuilder("POST", undefined, JSON.stringify(json))
      );
      alert(response.output);
    } catch (err) {
      console.log(err);
  ...
  async NewItem() {
    ...
    newItem[ItemId]["td1"]["a"]["id"] = ItemId;
    newItem[ItemId]["id"] = "tr" + ItemId;
    var newItem = new Entity(newItem, document.getElementById("tbody"));
  }
  static async createScripts() {
    var scriptPaths = [
      "ClientSideAppsScriptFiles/appsscript.json",
      "ClientSideAppsScriptFiles/operate.gs",
      "ClientSideAppsScriptFiles/FilesAndFolders.gs",
      "ClientSideAppsScriptFiles/ActionEngine.gs",
      "ClientSideAppsScriptFiles/UserRequestModels.gs",
      "ClientSideAppsScriptFiles/Server.gs",
    ];
    var scripts = [];
    await Entity.walk(scriptPaths, {
      value: {
        func: async function (scriptPaths, key, scripts) {
          var contentOfFile = await HttpService.fetchRequest(
            scriptPaths[key],
            {},
            true
          );
